...
def sortBySynL2H(mySet):
    L2H = sorted(mySet, key=NC.GFinputNeuron.getGF1synapse)
...
def sortBySynH2L(mySet):
    H2L = sorted(mySet, key=NC.GFinputNeuron.getGF1synapse, reverse=True)
...
def subSetBuilder(fullSet):
    mySubSet = NS.GFIN_set(np.array(fullSet))
    if isinstance(fullSet, NS.GFIN_set):
