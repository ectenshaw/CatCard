import DNInputNeuronClass
import numpy as np
import statistics as stat
import jsonNeurons as JN
import GetAnnotationsRemoveExtraneousInfo
import getSkeletonNodesNew as SKN
import GetDNrightConnectivity as DNC
import pandas
...
class DNIN_set(object):
    ...
    def __add__(self, otherList):
        ...
        for i in self.container:
            myList.append(i)
        if isinstance(otherList, DNInputNeuronClass.DNinputNeuron):
    ...
    def getConnectors(self):
        ...
        myConnectors = {}
        myConCount = {}
        connectorInfo = SKN.getAllConnectorInfo()
        while len(myBranches) is not 0:
            branch = myBranches.pop()
            myConnectors[branch] = SKN.getConnectorsByBranch(branch = branch, connectorInfo = connectorInfo)
    ...
    def getNumPartnersBySkid(self):
        ...
        for i in self:
            ...
            if 'postsynaptic to LC6' in i.annotations:
                ...
                d = {}
                a = DNC.getSkeletonPartners(int(i.skeletonID))
    ...
    def LC4_LO_Filter(self):
        ...
        for x in self:
            LOtnid = []
            LOtnid = SKN.getDownStreamNodes(skeletonID=x.skeletonID, tag='startOfLobula')
    ...
    def getConnectorsInfo(self):
        ...
        myConnectorInfo = DNC.getConnectorPartners(DNC.getConnectors(mySkids, 'pre'))
        return myConnectorInfo
        
        
#call this to build a DNIN_set from all DNright input neurons            
def builder(SKID):
    mySet = DNIN_set(np.array(DNInputNeuronClass.builder(SKID)))
    mySet.myDN = GetAnnotationsByID.getDNType(SKID)
    mySet.myDNskid = SKID
    return mySet

def buildFromSkidList(myList):    
    mySet = DNIN_set(np.array(DNInputNeuronClass.buildFromSkidList(myList)))
    return mySet

#sorts neurons from low to high synapse counts
def sortBySynL2H(mySet):
    L2H = sorted(mySet, key = DNInputNeuronClass.DNinputNeuron.getDNrightsynapse)
...
def sortBySynH2L(mySet):
    H2L = sorted(mySet, key = DNInputNeuronClass.DNinputNeuron.getDNrightsynapse, reverse=True)
...
def appendNonDNIN(mySet, annotation):
    ...
    for item in testList:
        if item not in curSKIDs:
            myNeuron = DNInputNeuronClass.buildSingleCell(int(item))
            mySet += myNeuron
    return mySet
def appendNonDNIN2(mySet, annotation):
    ...
    for item in testList:
        if item not in curSKIDs:
            myNeuron = DNInputNeuronClass.buildSingleCellQuick(int(item))
            mySet += myNeuron
    return mySet    
