...
class GFIN_set(object):
    ...
    def __add__(self, otherList):
        ...
        for i in self.container:
            myList.append(i)
        if isinstance(otherList, GFInputNeuronClass.GFinputNeuron):
...
def builder():
    mySet = GFIN_set(np.array(GFInputNeuronClass.builder()))
    mySet._medSyn()
    mySet.minMax()
    return mySet
...
def buildFromSkidList(myList):
    mySet = GFIN_set(np.array(GFInputNeuronClass.buildFromSkidList(myList)))
    return mySet
