...
import GFInputNeuronClass as NC
...
default_path = "/home/emily/Desktop/pyCharmOutputs/CSV/Cytoscape"
json_path = "/home/emily/Desktop/pyCharmOutputs/JSON"
gradient_path =  "/home/emily/Desktop/pyCharmOutputs/Gradients"

def createCytoscapeCSV(mySet, classification):
    copyNumber = 0
    fileName = classification + " cytoscape"
    pathVar = default_path
...
def makeBranchColorGradient(mySet, allInfo, branch):
    ...
    H2L = sorted(neurons, key=NC.GFinputNeuron.getGF1synapse, reverse=True)
    ...
    pathVar = str(gradient_path + "/Branches")
    myPath = "{}/".format(pathVar)
    myPath = os.path.normpath(myPath)
    if not os.path.isdir(myPath):
        os.makedirs(myPath)
...
def makeDistributionColorGradient(mySet, allInfo, distribution):
    ...
    nameVar = fileName
    myFile = str(fileName + "H2L.json")
    pathVar = str(gradient_path + "/Distribution")
    myPath = "{}/".format(pathVar)
    myPath = os.path.normpath(myPath)
    if not os.path.isdir(myPath):
        os.makedirs(myPath)
