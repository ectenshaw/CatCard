...
class DnAutoseg_set(object):
    ...
    def __add__(self, otherList):
        ...
        for i in self.container:
            myList.append(i)
        if isinstance(otherList, DescendingNeuronClass.DescendingNeuron):
...
def builder():
    ...
    for i in putative_skidlist:
        if i not in skidList:
            skidList.append(i)
    mySet = DnAutoseg_set(np.array(DescendingNeuronClass.builder(skidList)))
    return mySet
