...
def makeJsonFile(mySet):
    if type(mySet) is not NS.DNIN_set:
        mySet = NS.buildFromSkidList(mySet)
...
def getAdditionalInfo(mySet):
    myQuant = str(NS.quantiles(mySet))
...
def makeJsonOpacity(myNeurons):
    ...
    synapticCluster = NS.makeDistributedGroupsByColor(myNeurons)
